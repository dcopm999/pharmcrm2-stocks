...
class BatchPharmItem(models.Model):
    ...
    def get_absolute_url(self):
        return reverse("stocks:batch-pharmitem-detail", args=[str(self.slug)])

    def order_create(self):
        order = Order(
            incoming=self.batch.stock,
            pharm_item_id=self.id,
    ...
    def order_update(self):
        order = Order.objects.get(incoming=self.batch.stock, pharm_item_id=self.id)
    ...
    def save(self, *args, **kwargs):
        ...
        if Order.objects.filter(
            incoming=self.batch.stock, pharm_item_id=self.id
        ).exists():
            self.order_update()
        else:
            self.order_create()
    ...
    class Meta:
        verbose_name = _("Pharmaceutical batch item")
...
class Balance(models.Model):
    ...
    pharm_item = models.ForeignKey(
        BatchPharmItem,
        on_delete=models.CASCADE,
        verbose_name=_("Pharmaceutical batch items"),
    )
    ...
    def price_sum_original(self):
        return self.quantity_original * self.pharm_item.price_original

    def price_sum_item(self):
        return self.quantity_item * self.pharm_item.price_item

    def price_sum(self):
        return self.price_sum_original() + self.price_sum_item()

    def __str__(self):
        return f"{self.stock.name}({self.pharm_item})"
...
class Order(models.Model):
    ...
    pharm_item = models.ForeignKey(
        BatchPharmItem,
        on_delete=models.CASCADE,
        related_name="pharm_item",
        verbose_name=_("Pharm item"),
    )
    quantity_original = models.PositiveIntegerField(
    ...
    def __str__(self):
        return f"{self.pharm_item}({self.outgoing}->{self.incoming})"

    def balance_incoming(self):
        if not self.is_done:
            try:
                self.balance = Balance.objects.get(
                    stock_id=self.incoming_id, pharm_item_id=self.pharm_item_id
                )
                self.balance.stock = self.incoming
                self.balance.pharm_item_id = self.pharm_item_id
                ...
                self.balance = Balance(
                    ...
                    pharm_item_id=self.pharm_item_id,
                    quantity_original=self.quantity_original,
                    quantity_item=self.quantity_item,
                )
                self.balance.save()
