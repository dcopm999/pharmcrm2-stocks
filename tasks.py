...
def coverage(c):
    ...
    """
    c.run("coverage run --source pharmcrm2-stocks runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/pharmcrm2-stocks.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ stocks")
...
def lint(c):
    ...
    """
    c.run("flake8 pharmcrm2-stocks tests")
...
def release(c, bumpsize=""):
    ...
    c.run(
        "git tag -a {version} -m 'New version: {version}'".format(
            version=pharmcrm2_stocks.__version__
        )
    )
    c.run("git push --tags")
